...
ユーザーから「セクション追加して」「この比較軸を外して」のような追記・修正依頼が来た場合は、`plan` と `publish` のどちらを先に更新するかを以下で判断してください。

- **構成変更** の場合: 先に `zenn\plan` を更新する
- **軽微な本文修正** の場合: `zenn\publish` を直接更新してよい
- **publish 直編集で見出し構成や主張が変わる** 場合: 同じターンで `zenn\plan` も同期する
...
`plan` が存在する記事では、**見出し構成またはセクションの主張が変わったら、最終的に `plan` と `publish` を一致させる** ことを必須ルールにしてください。
...
3. `search_topic.py` または `research_topic.py` で、概要提案に必要な最小限の最新情報だけ集める
4. 概要提案を出す
...
7. 構成案を `zenn\plan` に出す
8. ユーザーフィードバックで構成が変わるなら、先に `zenn\plan` を更新する
9. セクションごとの根拠 URL を揃える
10. 必要な部分だけサブエージェントも活用しつつ記事本文を `zenn\publish` に書く
11. `zenn\publish` を直接更新した場合も、見出し構成や主張が変わったら `zenn\plan` を同期する
