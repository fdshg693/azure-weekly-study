...
# Entra ID 認証（App Registration の自動セットアップ）
just auth-setup   # App Registration を az CLI で作成/更新し auth.auto.tfvars に値を書き出す
just auth-show    # 現在の App 状態・公開スコープ・許可済スコープを確認（読み取り専用）
...
`az` を直接 justfile に書くと巨大ワンライナーになるため、認証系コマンドは
[scripts/](scripts/) 配下の PowerShell に分離している:

| スクリプト | 役割 |
| --- | --- |
| [scripts/setup-entra-app.ps1](scripts/setup-entra-app.ps1) | App Registration 作成・リダイレクト URI・ログアウト URL・シークレット・Graph `User.Read`・Expose an API(`access_as_user`)・事前承認・SP をまとめて設定し `auth.auto.tfvars` を生成 |
| [scripts/show-entra-app.ps1](scripts/show-entra-app.ps1) | アプリの現状（公開スコープ・要求権限・シークレット有効期限・**同意済みスコープ**）を表示する確認用 |
| [scripts/destroy-entra-app.ps1](scripts/destroy-entra-app.ps1) | App Registration と `auth.auto.tfvars` を削除 |
| [scripts/_common.ps1](scripts/_common.ps1) | 上記の共通ヘルパー（Graph の固定 ID・PATCH ユーティリティ等） |
...
> **自動セットアップ（推奨）**: `just auth-setup` を実行すると、以下の手動手順
> （リダイレクト URI・ログアウト URL・シークレット・`User.Read`・OBO 用の
> Expose an API まで）をすべて az CLI で行い、`auth.auto.tfvars` を生成する。
> その後 `just up` で App Settings に反映、`just auth-show` で状態を確認できる。
> 以下は「内部で何をしているか」を理解するための手動手順リファレンス。
...
> `just auth-setup` を使った場合、この値は `auth.auto.tfvars` に自動生成されるため
> 手書き不要。`*.auto.tfvars` は Terraform が自動で読み込む。手動でやる場合は
...
### App Registration の追加設定

> **自動セットアップ済み**: 以下の OBO 用設定（Expose an API の `access_as_user`
> 公開・Authorized client applications への自分自身の追加・`/auth/redirect-obo`
> リダイレクト URI）は `just auth-setup` がまとめて行う。`just auth-show` の
> 「Expose an API」「Authorized client applications」セクションで確認できる。
> 以下は手動で行う場合のリファレンス。
