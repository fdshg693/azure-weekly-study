...
- **ステップ4: 管理 UI（画面）統合**（[PLAN.md](PLAN.md) §4 ステップ4）
    - `GET /` に EJS テンプレートの 1 画面を追加（[app/routes/pages.js](app/routes/pages.js) / [app/views/index.ejs](app/views/index.ejs)）
    - 「デプロイ一覧 / 新規デプロイ / 削除 / チャット試し打ち」を 1 画面に統合。画面の JS が**ステップ1〜3 の既存 JSON API をそのまま fetch** する（ロジックは作り直さない）
    - 作成直後の `Accepted`/`Creating` は画面が自動ポーリングし、`Succeeded` になったらチャットの選択肢に出る → **作ったモデルをその場で呼んで結果が返る**を 1 画面で体験
    - コントロールプレーン（管理）とデータプレーン（推論）の 2 面が、同じ画面の別セクションとして並ぶ構造を視覚的に確認できる

## 今後の予定

- 様々なAIモデルを管理画面から簡単にデプロイ（コントロールプレーン）← ステップ2「一覧」・ステップ3「作成/削除」・ステップ4「管理 UI 統合」で**実装完了**
- デプロイしたAIモデルをAPI経由で呼び出し可能（データプレーン）← ステップ1で最小版、ステップ4で管理 UI のチャット試し打ちに統合
- 残るは [PLAN.md](PLAN.md) §4 ステップ5（叩いて挙動が変わることを体験する学習の仕上げ）と、§7 の将来拡張（外部プロバイダ / APIキー管理 / APIM ベース AI Gateway 等）
...
## セットアップ（ステップ4：管理 UI 統合）

前提:
- ステップ0〜3 のリソース・ロールが揃っていること（推論=OpenAI User / 管理=Contributor）。
- `app/.env` に `AZURE_OPENAI_ENDPOINT` と `AZURE_OPENAI_ACCOUNT_ID` が入っていること（`just app-env-sync`）。

実行:

```pwsh
just app-install     # 依存をインストール（EJS を追加）
just app-env-sync    # endpoint + account-id + 既定 deployment を app/.env に書き出し
just app-dev         # アプリ起動（要 az login。既定ポート 3000）
just app-open        # 既定ブラウザで http://localhost:3000/ を開く（手動で開いてもよい）
```

ブラウザの 1 画面で以下が完結する（CLI（`just gw-*` / `just infer`）と**同じ API** を画面から叩く）:

1. **デプロイ一覧** … 作成済みデプロイと状態を表示。`Succeeded` でないものがあれば自動でポーリング更新。各行の「削除」ボタンで削除。
2. **新規デプロイ作成** … ベースモデルを選ぶと version / SKU 候補が連動。容量(TPM)とデプロイ名を入れて作成。
3. **チャット試し打ち** … `Succeeded` のデプロイを選んでプロンプトを送信 → 推論結果が返る。

> 「作成 → 一覧が自動更新で `Succeeded` → そのまま試し打ちで応答が返る」までを画面だけで体験できる。
> 同じ画面に**管理（コントロールプレーン）と推論（データプレーン）の 2 面**がセクションとして並ぶのが要点。

### 叩いて挙動が変わることを体験（ステップ4 版）

- 作成直後は状態バッジが黄色の `Creating` → しばらくして緑の `Succeeded` に変わり、チャットの選択肢に出現する（画面のポーリングで自動更新）。
- 容量(TPM)をクォータ超過で作成 → 画面上部の通知バナーに ARM の **4xx** がそのまま表示される。
- 管理ロールを外す → 一覧/作成/削除が **403** になり、一方チャット試し打ち（推論）は通る、という 2 面の分離を 1 画面で観察できる。
