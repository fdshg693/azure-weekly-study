...
## container — マネージドコンテナ（ACI / Web App for Containers / Container Apps）

`./container`（詳細: [container/CLAUDE.md](./container/CLAUDE.md)、計画: [container/PLAN.md](./container/PLAN.md)）
技術: Bicep / **Taskfile** / Azure CLI、サンプルは nginx 静的イメージ。

「**自作イメージを、オーケストレーションを自前で持たずにマネージド計算へそのまま載せる**」が軸。
local/docker（イメージを作る）と k8s（フルオーケストレーション）の**あいだ**を埋め、
スペクトラム **ACI → Web App for Containers → Container Apps（App 側）→ AKS** の使い分けを学ぶ。
`registry`（**ACR が主役・後続の土台**。Bicep で ACR(admin user 既定無効＝キーレス)＋消費者 UAMI＋AcrPull を作り、
最小イメージを `az acr build` でクラウドビルド。**repository/tag/digest の構造**、**同タグ上書きで digest が変わる**＝
tag は動く参照・digest は不変、**admin user（共有パスワード）vs Entra トークン認証**、AcrPull/AcrPush の認可分離を体感）→
`aci`（**ACI が主役・Step 2**。registry の ACR/イメージ/消費者 UAMI を参照し、**`imageRegistryCredentials.identity` に UAMI を
指定して keyless pull**＋ Public IP/FQDN 到達。**restartPolicy（Always/OnFailure/Never）**で再起動の有無、**AcrPull 剥奪→
作り直し→pull 403**（UAMI 本人で認証/認可の分離を回収）、**コンテナグループ同居＝localhost 共有**、**per-second 課金は消すと止まる**
＝Container Group＝Pod の素朴版を体感）。Step 3 以降（Web App for Containers / Container Apps）は未着手。
