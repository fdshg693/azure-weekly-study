...
- `modules/apim-crud-api.bicep`: CRUD API リソース定義（operation は `crud-api.operations.json`、policy は `crud-api.policy.xml` を読み込む）
- `modules/apim-aoai-api.bicep`: Azure OpenAI 用 API リソース定義（operation は `aoai-api.operations.json`、policy は `aoai-api.policy.xml` を読み込む）
- `modules/crud-api.operations.json` / `modules/crud-api.policy.xml`: CRUD API の operation 一覧と APIM policy
- `modules/aoai-api.operations.json` / `modules/aoai-api.policy.xml`: Azure OpenAI API の operation 一覧と APIM policy
...
Azure OpenAI を APIM に追加する場合は、`main.local.bicepparam` などで `enableAzureOpenAiApi = true` を指定します。リージョン・モデル・デプロイ SKU などは `azureOpenAi` パラメータ（object）で一括上書きできます。`japaneast` で `gpt-4o-mini` を使う場合は、`deploymentSkuName: 'GlobalStandard'` が必要です。設定例は `main.local.bicepparam.example` 参照。
