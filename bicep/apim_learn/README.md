...
Azure OpenAI を APIM に追加する場合は、`main.local.bicepparam` などで `enableAzureOpenAiApi = true` を指定します。必要に応じて `azureOpenAiLocation`、`azureOpenAiDeploymentName`、`azureOpenAiModelName`、`azureOpenAiModelVersion`、`azureOpenAiDeploymentSkuName`、`azureOpenAiDeploymentCapacity` を上書きしてください。
