...
この `x-backend-auth` は APIM のサブスクリプションキーとは別物です。利用者が APIM に送る `X-API-Key` や Function App の host key / function key では通りません。Function App を直叩きする場合は、`BACKEND_SHARED_SECRET` の値をそのまま `x-backend-auth` に送る必要があります。

この共有シークレットは secure パラメータです。未指定時はデプロイ時に自動生成され、固定値が必要な場合だけ `main.local.bicepparam` で上書きします。

`backendSharedSecret` を未指定のまま再デプロイすると値が変わるため、`test.local.http` に手入力した古い値では 401 になります。現在の値は `just backend-secret` で取得できます。固定値で運用したい場合は `main.local.bicepparam` で `backendSharedSecret` を明示してください。

## APIM と Azure OpenAI の接続

Azure OpenAI を APIM 配下へ追加する場合は、Bicep が Azure OpenAI リソース本体とモデルデプロイを作成し、そのエンドポイントとキーを APIM へ渡して別 API として公開します。
